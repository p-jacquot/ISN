...
class Dialog:
    ...
    def __init__(self, pathImg1, nom1, pos1, pathImg2, nom2, pos2):
        self.characters = [[nom1, pathImg1, pos1], [nom2, pathImg2, pos2]] #La liste des personnages, au cas où il y aurait plus de 2 perso's dans la conversation.
    ...
    def getPunchline(self):
        if self.counter < len(self.punchlineList):
            ...
            punchline = [self.punchlineList[self.counter][0],
                            self.punchlineList[self.counter][1]]
            self.counter += 1
            if self.counter >= len(self.punchlineList):
